...
public class Functions {
    ...
    public static double[][][] DefineEdges(int[][] tocke) { //TODO Jernej
        // {tocka1, ...}
        return new double[][][]{{{0, 0}, {1, 1}}, {{1, 1}, {3, 1}}, {{1, 1}, {2, 2}}, {{3, 1}, {2, 2}}, {{3, 1}, {4, 0}}}; // {{tocka1, tocka2}, ...}
    }
    ...
    public static Filter GenerateFilter(double edges[][][], int numberOfStages) {
        ...
        for (double[][] edge : edges) {
            ...
            filter.stages[(int) (numberOfStages * Math.max(edge[0][1], edge[1][1]))].edges.add(new Integer[]{dict.get(edge[0]), dict.get(edge[1])});
        }
        return filter;
    }
    ...
    public static double[][][] GenerateBarcode(Filter filter) {
        ...
        for (int i = 0; i < filter.stages.length; i++) {
            ...
            complex.add(new double[][]{
                ...
                {Integer.MAX_VALUE - 2, Integer.MAX_VALUE - 1}, {Integer.MAX_VALUE - 2, Integer.MAX_VALUE}, {Integer.MAX_VALUE - 1, Integer.MAX_VALUE},
                {Integer.MAX_VALUE - 2, Integer.MAX_VALUE - 1, Integer.MAX_VALUE}}, new double[]{
            ...
            */
            for (Iterator<Integer[]> it = stage.edges.iterator(); it.hasNext(); ) {
            	Integer[] f = it.next();
            	complex.add(new double[][]{{f[0].doubleValue(), f[1].doubleValue()}}, new double[]{i});
            }
            /*
            Integer[][] edges = stage.edges.toArray(new Integer[stage.points.size()][2]);
            if (edges.length > 0) {
            	int[][] eds = new int[edges.length][2];
        ...
        /*
         for (int i = 0; i < arrayOfBarcodes.length; i++) {
	         for (int j = 0; j < arrayOfBarcodes[i].length; j++) {
		         for (int k = 0; k < arrayOfBarcodes[i][j].length; k++) {
			         System.out.println("betti-" + i + ": bar:" + j + " " + arrayOfBarcodes[i][j][k]);
		         }
	         }
         }
         */
    ...
    private static boolean CompareTwoBarcodes(double[][][] bars1, double[][][] idealBars1) {
        ...
        for (int i = 0; i < Math.min(idealBars.length, bars.length); i++) {
            for (int j = 0; j < idealBars[i].length; j++) {
                ...
                if (((0.9 * start < zacetak) && (zacetak < 1.1 * start)) && ((0.9 * end < konec) && (konec < 1.1 * end))) {
    ...
    public static char CompareBarcode(ArrayList<double[][][]> bars, double[][][][][] idealneCrke) {//TODO Marija
        char detektovanaCrka = ' ';
        for (int i = 0; i < idealneCrke.length; i++) {
            double[][][][] crka = idealneCrke[i];
            if ((CompareTwoBarcodes(bars.get(0), crka[0])) && (CompareTwoBarcodes(bars.get(1), crka[1])) && (CompareTwoBarcodes(bars.get(2), crka[2])) && (CompareTwoBarcodes(bars.get(3), crka[3]))) {
                int index = i;
                if (index == 0) {
                    detektovanaCrka = 'A';
                } else if (index == 1) {
    ...
    public static char CompareBarcode2(double[][][][] bars, double[][][][][] idealneCrke) {//TODO Marija
        ...
        for (int i = 0; i < idealneCrke.length; i++) {
            double[][][][] crka = idealneCrke[i];
            if ((CompareTwoBarcodes(bars[0], crka[4])) && (CompareTwoBarcodes(bars[1], crka[5])) && (CompareTwoBarcodes(bars[2], crka[6])) && (CompareTwoBarcodes(bars[3], crka[7]))) {
                int index = i;
                if (index == 8) {
                    detektovanaCrka = 'I';
                } else if (index == 9) {
    ...
    private static double[][][] removBars(double[][][] bars) {
    	...
    	for (int i=0;i<bars.length;i++) {
    		if (bars[i].length>l) l = bars[i].length;
    	}
